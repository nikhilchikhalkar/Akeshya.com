  ...
  return (
    <>
     ...
     <div className="flex mb-2 justify-center items-center mx-auto gap-4">
            ...
            <span className="font-semibold text-[#222222] text-3xl md:text-4xl">OUR PROCESS</span>
            <hr className=" w-10 md:w-20 h-[2px] my-8 bg-gray-200 border-0 rounded dark:bg-gray-700"></hr>
    </div>    
...

        {
            process.map((item, index) => {
                return(
                    ...
                    <div key={index} data-aos='fade-up' data-aos-delay='100' className=' group hover:bg-indigo-700  shadow-lg border rounded-xl p-8 bg-[#f6f6f6]'>
                    <h4 className=' group-hover:text-white cursor-pointer  group-hover:bg-indigo-700  text-xl font-semibold text-center mb-2 text-[#222222] '>{item.title}</h4>
                    <p className=' group-hover:text-white group-hover:bg-indigo-700  text-[#5E5E5E]'>{item.desc}</p>
                </div>
                )
            })
        }
